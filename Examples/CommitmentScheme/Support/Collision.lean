...
The file exposes the reusable ROM probability estimates used by the basic and
Merkle commitment proofs:

* birthday bounds for logged traces and cached random oracles;
* fresh-hit bounds for post-commit cache growth;
* cross-log bounds used after deterministic collision reductions.

The predicates themselves live in `Examples.CommitmentScheme.Support.Cache` and
`Examples.CommitmentScheme.Support.Logging`; this file contains the probability
inequalities over those predicates.
-/
