...
Bound expression: `MerkleTree.bindingWitnessErrorTerm C depth t` is the
conservative whole-cache term `(t + 2 * (depth + 1))^2 / (2 * |C|)`.
...
`t * (t - 1) / (2 * |C|) + (depth + 1)^2 / |C|`. The compact corollary is
`MerkleTree.binding_bound_textbook_conditioned` under
`2 * (depth + 1)^2 <= t`.
-/
